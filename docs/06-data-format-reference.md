...
| 1 | 25-40 效果值 | 单体低费卡基准 |
| 2 | 40-60 效果值 | 单体中费卡基准 |
| 3 | 60-80 效果值 | 高费复合/AOE 卡基准 |

### 价值计算规则

- **伤害/治疗**：1 点 = 1 效果值
- **护盾**：1 点 = 1 效果值
- **Buff/Debuff**：持续 2 回合的 buff，总价值 = 数值 × 回合数 × 0.6（持续效果折扣）
- **抽牌**：1 张牌 ≈ 15 效果值
- **AOE**：每个额外目标的价值按 0.5 倍计算
- **位移**（传送/交换）：额外 +10 效果值
- **自伤代价**：1 点自伤 ≈ -3 效果值
...
| 卡牌 | 费用 | 计算 | 总价值 | 符合？ |
|---|---|---|---|---|
| 火球术 | 2 | 30 伤害 | 30 | 偏低（可接受） |
| 瞄准射击 | 2 | 35 伤害 | 35 | ✓ |
| 烈焰风暴 | 3 | 30 AOE(约45) | 45 | 偏低（AOE 折扣大） |
| 谋略 | 1 | 抽1(15) | 15 | ✓ |
| 回响 | 2 | 抽2(30) | 30 | 偏低（可接受） |
| 治愈之光 | 2 | 25治疗 | 25 | 偏低（可接受） |
| 能量过载 | 0 | +2能量(20) - 5自伤(-15) | 5 | 需注意平衡 |
```
