...
卡牌系统中**不存在「施法者」概念**，所有卡牌由玩家从手牌中直接释放到目标角色（或区域）。
`Cards/CardEffect.gd` 中各函数的 `caster` 参数已废弃（始终为当前选中角色或队伍中第一个存活角色），新增卡牌效果时无需关心。

**亲和力系统不存在**，`_affinity_multiplier()` 始终返回 `1.0`，新增卡牌效果时无需调用。
