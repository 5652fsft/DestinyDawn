...
func show_value(value: int, is_heal: bool = false, is_shield: bool = false):
	...
	tween.tween_property(self, "position:y", position.y - 40, fade_duration).set_ease(Tween.EASE_OUT)
	tween.tween_property(self, "modulate:a", 0.0, fade_duration).set_delay(0.2)
