...
func setup(id: String, data: Dictionary):
	...
	$CardFront/SkillLabel.text = "技能·%s" % data.get("skill", "")
	...
	$CardBack/Scroll/VBox/SkillLabel.text = "技能·%s" % data.get("skill", "")
...
# 选中样式：青色高亮框（不灰化、无晕影、提升 z 不被遮挡）
func _apply_selected_style(selected: bool):
	if not _normal_panel_style:
		return
	if selected:
		# 从正常样式复制出高亮样式，避免污染正常样式（否则移除队伍后无法恢复）
		var hl: StyleBoxFlat = _normal_panel_style.duplicate()
...
func _on_hover_enter():
	...
	_hover_tween.tween_method(_set_full_scale, _full_scale, _base_scale * CardTheme.DECK_HOVER_SCALE, CardTheme.HOVER_TWEEN_SEC)
	_hover_tween.parallel().tween_property(self, "self_modulate", CardTheme.HOVER_MODULATE, CardTheme.HOVER_TWEEN_SEC)

func _on_hover_exit():
	if _hover_tween: _hover_tween.kill()
	z_index = 15 if _in_team else 5
	_hover_tween = create_tween().set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
