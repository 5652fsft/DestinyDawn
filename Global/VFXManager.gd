...
func _make_heal() -> GPUParticles2D:
	...
	p.amount = 12
	p.lifetime = 0.5
	p.process_material = _material(PRESET_COLORS.heal, 270, 30, Vector2(0, -60), 80)
	return p
...
func _make_shield() -> GPUParticles2D:
	...
	p.amount = 6
	p.lifetime = 0.4
	var m = _material(PRESET_COLORS.shield, 0, 360, Vector2(), 60)
	m.scale_min = 0.3
	m.scale_max = 1.0
...
func _make_buff() -> GPUParticles2D:
	...
	p.amount = 10
	p.lifetime = 0.5
	p.process_material = _material(PRESET_COLORS.buff, 0, 360, Vector2(0, -40), 100)
	return p
...
func _make_debuff() -> GPUParticles2D:
	...
	p.amount = 8
	p.lifetime = 0.55
	var m = _material(PRESET_COLORS.debuff, 0, 360, Vector2(), 70)
...
func _make_entrance() -> GPUParticles2D:
	...
	p.amount = 16
	p.lifetime = 0.4
	p.process_material = _material(PRESET_COLORS.entrance, 270, 45, Vector2(0, -80), 120)
	return p
...
func _make_explosion() -> GPUParticles2D:
	...
	m.initial_velocity_max = 250
	m.scale_min = 0.5
	m.scale_max = 2.0
	m.hue_variation_min = 0.0
	m.hue_variation_max = 0.1
	p.process_material = m
	return p
